...
class ScanNetDataset_sample_graph_edge(Dataset):
    def __init__(self,
        ...
        self.tokenizer = RobertaTokenizerFast.from_pretrained(os.path.join(self.data_root, 'roberta-base/'), local_files_only=True)
    ...
    def __getitem__(self, index: int) -> Tuple:
        ...
        for i in range(len(scene_input)):
            ...
            if i==0:
                ...
                feat_pth = os.path.join(self.data_root,'clip_feat/'+scan_id+'.pth')
                feat_2d = torch.load(feat_pth)[torch.tensor(unique).long()]
            ...
            # /bak_home/songcl/3D_visual_grounding/data/ipdn_data/data/clip_feat
            ...
            context_label = set()
            # print(lang_utterance)
            # print(len(lang_utterance.split(' ')))
            # # print('\n')
            # pp
    ...
    def collate_fn(self, batch: Sequence[Sequence]) -> Dict:
        ...
        # print(lang_masks.size())
        # print(lang_tokenss[0])
        # print(lang_tokenss.size())
        # pp
        ...
        # print(ann_ids)
        # ann_ids = torch.tensor(ann_ids)  # int [B+1]
        # scan_ids = torch.tensor(scan_ids)  # int [B+1]
        spatial_shape = torch.tensor(spatial_shape) 
        # object_idss = torch.tensor(object_idss) 
        # gt_pmasks = gt_pmasks[0].unsqueeze(0)
        # gt_spmasks = gt_spmasks[0].unsqueeze(0)
        # sp_ref_masks = sp_ref_masks[0].unsqueeze(0)
        # sp_ins_labels = sp_ins_labels[0].unsqueeze(0)
        # print(sp_ins_labels)
        # meta_datas = torch.tensor([])
        # view_dependents = torch.tensor([])
        # dense_mapss =torch.tensor([])
        #  dense_mapss[0]
        # for k,v in dense_maps[0].items():
        #     print(k)
        #     globals()[k] = dense_maps[v]
        #     print(type(globals()[k]))
     
        # print(view_dependents)
        # pp
        batch = {
            'ann_ids': ann_ids,#
            'scan_ids': scan_ids,
