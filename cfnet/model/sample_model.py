...
class SamplingModule(nn.Module):
    ...
    def forward(self, pc_feat, lang_feat, pc_mask, lang_mask, gate = None):
        """
        Args:
            features: (B, N, D)
            lang_feat: (B, L, D)
        ...
        # (B, N, L)
        ref_scores = self.camap(pc_feat, lang_feat, pc_mask, lang_mask.bool()) # (B, N, L)
        ref_scores = ref_scores * (~lang_mask.unsqueeze(1)).float() # (B, N, L)
        ref_scores = ref_scores.sum(-1)   # [B, N]
        if gate != None:
            ref_scores = ref_scores * gate
        # print(ref_scores)
